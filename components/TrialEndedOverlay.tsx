...
export const TrialEndedOverlay: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center" style={{ background: 'rgba(15, 23, 42, 0.85)', backdropFilter: 'blur(8px)' }}>
      <div className="w-full h-full overflow-y-auto py-8 px-4">
        <div className="max-w-5xl mx-auto">
          {/* Trial Ended Message */}
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full mb-4" style={{ background: 'rgba(245, 158, 11, 0.15)', border: '2px solid rgba(245, 158, 11, 0.3)' }}>
              <AlertTriangle size={32} className="text-amber-500" />
            </div>
            <h1 className="text-3xl lg:text-4xl font-bold text-white mb-3" style={{ fontFamily: 'Syne, sans-serif' }}>
              Your Trial Has Ended
            </h1>
            <p className="text-slate-300 text-lg max-w-lg mx-auto">
              Kindly subscribe to continue using FreightIntel. Choose a plan below to unlock full access.
            </p>
          </div>
              ...
              return (
                <div
                  ...
                  className={`relative flex flex-col rounded-2xl p-8 transition-all duration-300 ${
                    plan.popular
                      ? 'text-white scale-[1.03] z-10 shadow-xl'
                      : 'bg-white/95 border border-slate-200 shadow-sm'
                  }`}
                  style={plan.popular ? { background: 'linear-gradient(135deg, #7C5CFC, #9B7EFD)', boxShadow: '0 20px 60px rgba(124,92,252,0.3)' } : undefined}
                >
                  {plan.popular && (
                  ...
                  <div className="mb-6">
                    <h3 className={`text-xl font-bold mb-2 ${plan.popular ? 'text-white' : 'text-slate-900'}`} style={{ fontFamily: 'Syne, sans-serif' }}>
                      {plan.name}
                    </h3>
                  ...
                  <button
                    ...
                    style={plan.popular ? { color: '#7C5CFC' } : { background: 'linear-gradient(135deg, #7C5CFC, #9B7EFD)' }}
                  >
                    {plan.cta}
                  </button>
          ...
          <div className="text-center">
            <button
              onClick={() => window.open('https://api.whatsapp.com/send?phone=966591860764', '_blank')}
              className="inline-flex items-center gap-2 px-8 py-4 rounded-xl text-white font-bold text-lg transition-all hover:opacity-90 shadow-lg"
              style={{ background: 'linear-gradient(135deg, #7C5CFC, #9B7EFD)', boxShadow: '0 8px 32px rgba(124,92,252,0.3)' }}
            >
              Contact Support
            </button>
