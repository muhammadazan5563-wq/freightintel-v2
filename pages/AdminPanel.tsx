                  ...
                  filteredUsers.map((user) => (
                    <tr key={user.id} className={`hover:bg-slate-50 transition-colors ${user.isBlocked ? 'opacity-50 bg-red-50/50' : ''}`}>
                      ...
                      <td className="p-4">
                        <button
                          onClick={async () => {
                            const updated = { ...user, hasMailerAccess: !user.hasMailerAccess };
                            const success = await updateUserInSupabase(updated);
                            if (success) {
                              setUsers(users.map(u => u.id === user.id ? updated : u));
                              showMessage('success', updated.hasMailerAccess ? 'Mailer access granted' : 'Mailer access revoked');
                            } else {
                              showMessage('error', 'Failed to update mailer access');
                            }
                          }}
                          disabled={isSaving}
                          className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border transition-all disabled:opacity-50 ${
                            user.hasMailerAccess
